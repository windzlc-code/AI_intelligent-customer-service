...
class CustomerServiceStore:
    ...
    def claim_conversation(self, conversation_id: int, admin_id: int, reply_source: str = "") -> dict[str, Any]:
        ts = now_ts()
        with db() as conn:
            row = conn.execute("SELECT * FROM conversations WHERE id = ?", (int(conversation_id),)).fetchone()
            if row is None:
            ...
            conn.execute(
                """
                INSERT INTO admin_sessions(admin_telegram_id, current_conversation_id, current_reply_source, updated_at)
                VALUES (?, ?, ?, ?)
    ...
    def set_admin_current_conversation(self, conversation_id: int, admin_id: int, reply_source: str = "") -> dict[str, Any]:
        ts = now_ts()
        with db() as conn:
            row = conn.execute("SELECT * FROM conversations WHERE id = ?", (int(conversation_id),)).fetchone()
            if row is None:
    ...
    def list_handoff_processing_conversations(self) -> list[dict[str, Any]]:
        marker_values = (
            PAYMENT_BUTTON_TEXT,
            FEEDBACK_BUTTON_TEXT,
            OTHER_BUTTON_TEXT,
            ...
            cur = conn.execute(
                ...
                WHERE conversation_id = ?
                  ...
                  AND COALESCE((
                    SELECT mt.text
                    FROM messages mt
                    WHERE mt.conversation_id = messages.conversation_id
                      AND mt.direction = 'user'
                      AND mt.message_type IN ('callback', 'command')
                      AND mt.text IN (?, ?, ?, ?, ?, ?)
                      AND mt.id < messages.id
                    ORDER BY mt.id DESC
                    LIMIT 1
                  ), '') NOT IN (?, ?)
                """,
